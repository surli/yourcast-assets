...
loadLess(LESS_ROOT+"/irsam_render_ICalReader_for_irsam.less");
...
function irsam_render_ICalReader_for_irsam(collection, zone, timeInfo) {
	...
	if (xmlhttp.readyState == 4 && xmlhttp.status==200){
		...
		console.log("events ...." + eventsByDays);
		...
		for (var idays = 0; idays < daysSort.length; idays++) {
			...
			for (var indice = 0; indice < elementsArray.length; indice++) {
				...
				if(pastEvent(elements)==false){				
					...
					if (inProgress(elements) == true){title = "<div style=\"color: #9d261d;\"><b>"+elements.summary+"</b></div>";}

					if (inProgress(elements) == false){title = "<b>"+elements.summary+"</b>";}
...
function getLocation(location, doc) {
    ...
    if (lieu.indexOf(" ")!=-1){lieu=lieu.replace(" ","");}	// R�gler le probl�me d'espace apr�s la parenth�se
