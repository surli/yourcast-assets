...
function render_date_edt_synth(elements) {
    if (elements.start == 0 || elements.end == 0){
        ...
        if(start.month == end.month && start.day == end.day){
            var startTime = start.hour + ":" + start.min;
            var endTime = end.hour + ":" + end.min;
            var content = "";
            content = "<em>"+startTime+"   "+endTime+"</em>";
            return content;
        }
        else {
            if(start.hour == 00 && start.min == 00 && end.hour == 00 && end.min == 00) {
                ...
                if(end.day == today.day && end.month==today.month)
                    endDate = "Aujourd'hui";
                var content = "";
                content = "<em>"+startDate+" &darr; "+endDate+"</em>";
                if(parseInt(elements.end)-parseInt(elements.start) == 86400000)
                    content = "<em>"+startDate+"</em>";
                return content;
            }else if(start.hour == 00 && start.min == 00 && end.hour != 00 && end.min != 00) {
                var startDate = start.day + " " + start.month;
                if(start.day == today.day && start.month==today.month)
                ...
                var endTime = end.hour + "h" + end.min;
                var content = "";
                content = "<em>"+startDate+" &rarr; "+endDate+" "+endTime+"</em>";
                return content;
            }else if(start.hour != 00 && start.min != 00 && end.hour == 00 && end.min == 00) {
                var startDate = start.day + " " + start.month;
                if(start.day == today.day && start.month==today.month)
                ...
                var endTime = end.hour + "h" + end.min;
                var content = "";
                content = "<em>"+startDate+" "+startTime+" &rarr; "+endDate+"</em>";
                return content;
            }else {
                var startDate = start.day + " " + start.month;
                if(start.day == today.day && start.month==today.month)
                ...
                var endTime = end.hour + "h" + end.min;
                var content = "";
                content = "<em>"+startDate+" "+startTime+" &rarr; "+endDate+" "+endTime+"</em>";
                return content;
            }
        }
    }
